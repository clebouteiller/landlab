...
class Space(Component):
    ...
    def __init__(self, grid, K_sed=None, K_br=None, F_f=None, 
        ...
        if discharge_field is not None:
            if type(discharge_field) is str:
                self.discharge_field = str(discharge_field)
            else:
                self.discharge_field = np.array(discharge_field)
        else:
            ...
            if self.discharge_method == 'drainage_area':
                ...
                if self.discharge_field is not None:
                    if type(self.discharge_field) is str:
                        ...
                        self.Q_to_the_m[:] = np.power(self.q, self.m_sp)
                        #print('mean discharge in ha is '+repr(np.mean(self.q)))
                    else:
    ...
    def run_one_step(self, dt=1.0, flooded_nodes=None, \
                     discharge_field_if_used=None,\
                     K_sed_if_used=None,sp_crit_sed_if_used=None,**kwds):
        ...
        """        
        #Dynamic update of discharge field if needed
        #The same should be done for area field (in case drainage system evolves)
        if discharge_field_if_used is not None:
            if type(discharge_field_if_used) is str:
                self.discharge_field = str(discharge_field_if_used)
            else:
                self.discharge_field = np.array(discharge_field_if_used)  
                
        if K_sed_if_used is not None:
            self.K_sed = K_sed_if_used
            
        if sp_crit_sed_if_used is not None:
            self.sp_crit_sed = sp_crit_sed_if_used
